...
- Generated the GitHub-tracked audit input file:
  ...
  It now contains the final expanded UI corpus with `2192` rows: Spanish `660`,
  French `911`, Russian `621`. The earlier `988`-row file covered only base
  sound-unit arrays.
- The audit input is now reproducible with `npm.cmd run ipa:audit:export`.
  ...
  current export keeps the same `2192` total rows and marks `36` deck focus
...
- Remaining audit work: continue from the new `2192`-row final UI corpus and
